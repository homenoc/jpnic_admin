...
class JPNICHandle(models.Model):
    ...
    name_en = models.CharField("名前(英語)", max_length=20, db_index=True)
    email = models.CharField("メールアドレス", max_length=50, db_index=True)
    org = models.CharField("組織名", max_length=30, db_index=True)
...
class V4List(models.Model):
    ...
    org = models.CharField("組織名", max_length=30)
    org_en = models.CharField("組織名(英語)", max_length=30, db_index=True)
    resource_admin_short = models.CharField("資源管理者略称", max_length=30)
    recep_number = models.CharField("受付番号", max_length=10)
    deli_number = models.CharField("審議番号", max_length=10, null=True, blank=True)

...
class JPNICHandle(models.Model):
    class Meta:
        ...
        index_together = [
            ...
            ["last_checked_at", "asn_id", "name"],
            ["last_checked_at", "asn_id", "name_en"],
        ]
    ...
    jpnic_handle = models.CharField("JPNICハンドル", max_length=20)
...
class V4List(models.Model):
    class Meta:
        ...
        index_together = [
            ["last_checked_at", "asn_id"],
            ["last_checked_at", "asn_id", "network_name"],
            ["last_checked_at", "asn_id", "address"],
            ["last_checked_at", "asn_id", "address_en"],
            ["last_checked_at", "asn_id", "org"],
            ["last_checked_at", "asn_id", "org_en"],
        ]
    ...
    admin_jpnic = models.ForeignKey(
        "data.JPNICHandle", related_name="v4lists", null=True, on_delete=models.CASCADE
    )
    tech_jpnic = models.ManyToManyField(JPNICHandle)
    admin_hash = models.CharField("管理者連絡窓口(ハッシュ値)", max_length=128)
    tech_hash = models.CharField("技術者連絡窓口(ハッシュ値)", max_length=128)
...
class V6List(models.Model):
    ...
    admin_jpnic = models.ForeignKey(
        "data.JPNICHandle", related_name="v6lists", null=True, on_delete=models.CASCADE
    )
    tech_jpnic = models.ManyToManyField(JPNICHandle)
    admin_hash = models.CharField("管理者連絡窓口(ハッシュ値)", max_length=128)
    tech_hash = models.CharField("技術者連絡窓口(ハッシュ値)", max_length=128)
